...
const ee = (e, t, r) => {
  ...
  return i.addEventListener("click", async (c) => {
    ...
    const d = b(String(e.id)), z = (e.inputs ?? []).filter((u) => u.link != null).map((u) => u.name), _ = p ?? { schema: "plenio.sheet_state/1", docs: {} }, S = d?.owned ?? Q(z, _), W = String(e.widgets?.find((u) => u.name === "review")?.value ?? "continue"), { openSheetDialog: j } = await import("./open-BQQk60Bf.mjs");
