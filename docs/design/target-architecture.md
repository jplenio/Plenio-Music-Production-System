...
**Phase 4A outcomes (2026-09-25, study report `docs/test-reports/2026-09-25-phase-4a.md`):** D-04 → **faster-whisper large-v3** (worker process of the host Python, VAD off, only over the transcription's vocal regions, fixed seed, on-disk cache); Qwen3-ASR not measured (download not approved). D-05 → separation **not built** in 4B (no measured ASR gain on the test material); delivered audio is never separated (owner, 2026-09-25). D-06 → adapter **on for instrumental covers** (lazy switch on `instrumental`; 0/6 flagged as vocal vs 2/7 without, all at the score length, equal or better melody identity) and **bypassed on the Song path** (very long or unfinished plans, abrupt endings) — both subject to the owner's listening verdict. The owner confirmed on 2026-09-25 that covers default to *instrumental*.
...
| **R11 Documented frontend APIs only** | Extensions use `registerExtension` hooks, `getCustomWidgets`, `addWidget`/`addDOMWidget`, widget values, `node.properties`, documented dialog/toast/settings/command APIs. No monkey-patching of internal stores or unpublished globals. *One recorded exception (Phase 4B):* Plenio nodes with a DynamicCombo wrap their own `configure` to repair the frontend 1.53.6 restore defect (yue2-cover-design §21.2); remove it when the frontend is fixed. | Survives frontend updates (Phase 1A §13 migration notes). |
...
| `GET /plenio/asr/notes/{draft_sha256}` | the Transcribe Lyrics note of a lyrics draft (unsure and left-out words), keyed by the draft's hash; `{"note": null}` when unknown (Phase 4B) |
...
| C1 | *Audio Repair* `PlenioRepair` (conditional) | Plenio/Mastering | 7 gate → **provisionally not built** (no clipping or HF defect measured; decision with unprocessed takes, Phase 7 report §5) | any audio | Only if measurement on YuE2/MiniMax output shows benefit (declip, HF de-harsh, spectral cleanup, filter as operations of one node). |
...
*Phase 4A decisions* ([yue2-cover-design.md](yue2-cover-design.md) §10): faster-whisper large-v3 in a worker subprocess of the host Python, VAD off, beam 5, word timestamps, only over the vocal regions of the transcription (`clip_timestamps`), fixed random seed; results cached on disk by source hash + vocal regions + engine + model revision + settings, so a ComfyUI restart reproduces the same draft (an edited lyrics document stays valid); alignment on the beat grid with the pickup rule; no separation.
...
| Instrumental AR LoRA (Mothersuperior) | CC BY-NC 4.0 [VF] | cover template: used for instrumental covers (Phase 4A); Song path: optional block; notice on the block and in the record |
...
| AS-07 | Instrumental LoRA keys map onto the native YuE2 CLIP (`text_encoders.model.layers.*`) without "lora key not loaded" warnings, and improve instrumental rate | **load verified (V3, Phase 4A)**: no unloaded-key warning; the same seed gives a different plan (219 → 0 vocal notes). **Effect**: 0 vocal notes in 4/4 valid text-path plans, but long plans (226–387 s), 2 of 6 plans cut at the planner's token limit (invalid) and abrupt endings (4/4); **covers**: 0/6 adapter takes flagged as vocal (2/7 without), all at the score length, melody overlap equal or higher (4A report §E5) → on for instrumental covers, bypassed on the Song path; musical judgement: owner listening pack | 4A |
| AS-08 | SheetSage2 re-transcription of a take is a usable vocal detector (FP/FN rates) | **measured (V3, Phase 4A)**: `score` (SheetSage2) and `listen` (Gemma 4) flag all 8 sung files and agree on 3 of the 4 instrumental takes `score` flags; the ASR word count is unusable (invents words on instrumentals). FP/FN rates need the owner's verdicts (thresholds *uncalibrated* until then) — 4A report §E3 | 4A, 4B |
...
| AS-18 | YuE2 sings supplied lyrics section by section | **mostly (Phase 4A)**: 2 of 3 sung text-path takes followed the lyrics (WER 6.7–8.6 %), one sang the chorus in the second verse's slot (32 %). **Phase 4B covers:** original lyrics WER 0 %; new lyrics depend on syllable fit and voice range (WER 1.14 -> 0.42, yue2-cover-design §21.1); the sheet warns before rendering | 4B |
| AS-19 | ASR drafts are reproducible, so an edited lyrics document stays valid across runs | **verified (Phase 4B)**: after a server restart the draft came from the disk cache with the same hash (real run, host tests). Phase 4A: faster-whisper's temperature fallback made two runs on an instrumental take differ (69 vs 36 words), and `temperature=0` alone loops on non-vocal audio; Plenio transcribes only vocal regions, fixes the seed and caches results on disk (the cache is the guarantee) | 4B |
